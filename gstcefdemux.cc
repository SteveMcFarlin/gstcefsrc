...
#define CEF_AUDIO_CAPS                                                         \
  "audio/x-raw, format=F32LE, rate=[1, 96000], channels=[1, "                  \
  "2], layout=interleaved"
...
G_DEFINE_TYPE_WITH_CODE(GstCefDemux, gst_cef_demux, GST_TYPE_ELEMENT,
                        GST_DEBUG_CATEGORY_INIT(gst_cef_demux_debug, "cefdemux",
                                                0, "cefdemux element"););
...
static GstStaticPadTemplate gst_cef_demux_audio_src_template =
    GST_STATIC_PAD_TEMPLATE("audio", GST_PAD_SRC, GST_PAD_ALWAYS,
                            GST_STATIC_CAPS(CEF_AUDIO_CAPS));

static gboolean gst_cef_demux_push_events(GstCefDemux *demux) {
  GstSegment segment;
  GstEvent *event;
  ...
  if (demux->need_caps) {
    ...
    audio_caps = gst_caps_new_simple(
        "audio/x-raw", "format", G_TYPE_STRING, "F32LE", "rate", G_TYPE_INT,
        demux->audio_rate, "channels", G_TYPE_INT, demux->audio_channels,
...
static GstClockTime gst_element_get_current_clock_time(GstElement *element) {
  GstClock *clock = NULL;
  GstClockTime ret;
...
static GstClockTime gst_element_get_current_running_time(GstElement *element) {
...
static gboolean gst_cef_demux_push_audio_buffer(GstBuffer **buffer, guint idx,
                                                AudioPushData *push_data) {

  push_data->demux->last_audio_time =
      gst_element_get_current_running_time(GST_ELEMENT_CAST(push_data->demux));
  GST_BUFFER_DTS(*buffer) = push_data->demux->last_audio_time;
  GST_BUFFER_PTS(*buffer) = push_data->demux->last_audio_time;

  // see: https://github.com/centricular/gstcefsrc/issues/73 for the reason for
  // this change. gst_buffer_add_audio_meta (*buffer,
  // &push_data->demux->audio_info, gst_buffer_get_size (*buffer), NULL);
  gst_buffer_add_audio_meta(
      *buffer, &push_data->demux->audio_info,
      gst_buffer_get_size(*buffer) /
          GST_AUDIO_INFO_BPF(&push_data->demux->audio_info),
      NULL);
  ...
  push_data->combined = gst_flow_combiner_update_pad_flow(
      push_data->flow_combiner, push_data->demux->asrcpad,
...
static void gst_cef_demux_update_audio_caps(GstCefDemux *demux,
  ...
  caps = gst_caps_new_simple("audio/x-raw", "format", G_TYPE_STRING, "F32LE",
                             "rate", G_TYPE_INT, rate, "channels", G_TYPE_INT,
                             channels, "channel-mask", GST_TYPE_BITMASK,
...
static GstFlowReturn gst_cef_demux_chain(GstPad *pad, GstObject *parent,
                                         GstBuffer *buffer) {
...
static gboolean gst_cef_demux_sink_event(GstPad *pad, GstObject *parent,
                                         GstEvent *event) {
  GstCefDemux *demux = (GstCefDemux *)parent;
  ...
  case GST_EVENT_CUSTOM_DOWNSTREAM: {
    ...
    if (gst_structure_has_name(s, "cef-audio-stream-start")) {
      demux->cef_audio_stream_start_events =
          g_list_append(demux->cef_audio_stream_start_events, event);
      event = NULL;
    }
    break;
  }
...
static gboolean gst_cef_demux_sink_query(GstPad *pad, GstObject *parent,
                                         GstQuery *query) {
  GstCefDemux *demux = (GstCefDemux *)parent;
  gboolean ret;
...
static void gst_cef_demux_init(GstCefDemux *demux) {
  GstPad *sinkpad =
      gst_pad_new_from_static_template(&gst_cef_demux_sink_template, "sink");
  ;

  demux->vsrcpad = gst_pad_new_from_static_template(
      &gst_cef_demux_video_src_template, "video");
  demux->asrcpad = gst_pad_new_from_static_template(
      &gst_cef_demux_audio_src_template, "audio");
  ...
  demux->audio_rate = DEFAULT_AUDIO_RATE;
  demux->audio_channels = DEFAULT_AUDIO_CHANNELS;
}

static void gst_cef_demux_set_property(GObject *object, guint prop_id,
                                       const GValue *value, GParamSpec *pspec) {
  GstCefDemux *demux = GST_CEF_DEMUX(object);

  switch (prop_id) {
  case PROP_AUDIO_RATE:
    demux->audio_rate = g_value_get_uint(value);
    break;
  case PROP_AUDIO_CHANNELS:
    demux->audio_channels = g_value_get_uint(value);
    break;
  default:
...
static void gst_cef_demux_get_property(GObject *object, guint prop_id,
                                       GValue *value, GParamSpec *pspec) {
  GstCefDemux *demux = GST_CEF_DEMUX(object);

  switch (prop_id) {
  case PROP_AUDIO_RATE:
    g_value_set_uint(value, demux->audio_rate);
    break;
  case PROP_AUDIO_CHANNELS:
    g_value_set_uint(value, demux->audio_channels);
    break;
  default:
    G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
    break;
...
static void gst_cef_demux_finalize(GObject *object) {
  GstCefDemux *demux = GST_CEF_DEMUX(object);
...
static void gst_cef_demux_class_init(GstCefDemuxClass *klass) {
  ...
  gobject_class->set_property = gst_cef_demux_set_property;
  gobject_class->get_property = gst_cef_demux_get_property;

  g_object_class_install_property(
      gobject_class, PROP_AUDIO_RATE,
      g_param_spec_uint("rate", "rate", "Initial audio sample rate ", 1, 96000,
                        DEFAULT_AUDIO_RATE,
  ...
  g_object_class_install_property(
      gobject_class, PROP_AUDIO_CHANNELS,
      g_param_spec_uint("channels", "channels", "Initial channels", 1, 2,
  ...
  gst_element_class_set_static_metadata(
      gstelement_class, "Chromium Embedded Framework demuxer",
      "Demuxer/Audio/Video", "Demuxes audio and video from cefsrc",
